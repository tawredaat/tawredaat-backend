					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="m-portlet">
								...
								<!-- Multistep Form -->
								<div class="main">
								<form action="{{route('company.selectedL1Categories')}}" class="regform" method="get">
									@csrf
								<input type="hidden" name="brand" value="{{request('brand_id')}}">
								<!-- Progress Bar -->
								<ul id="progressbar">
								<li>Brand</li>
								...
								<fieldset id="first" style="width: 100%">
									<h2 class="title">Select category(level one)</h2>
									...
									<div class="form-group">
										<select required="" multiple="" style="height: 20px" data-live-search="true" data-actions-box="true"  id="idSelect" name="categories[]" id="" class="form-control selectpicker ">
											@foreach($l1Categories as $category)
												<option value="{{$category->id}}">{{$category->translate('en')->name}}-{{$category->translate('ar')->name}}</option>
											@endforeach
										</select>
									</div>
									<input class="pre_btn" onclick="window.location='{{ route('company.products.selectBrands') }}'" type="button" value="Previous">
