...
@section('reports-general-rfq-active', 'm-menu__item--active m-menu__item--open')
@section('page-title', 'Reports|General RFQs')
@section('content')
    <style type="text/css">
        .swal2-confirm {
                ...
                <div class="m-portlet m-portlet--mobile">
                    ...
                    <div class="m-portlet__body">
                        ...
                        <table class="table table-striped- table-bordered table-hover table-checkable"
                            ...
                            @if(count($general_rfqs) > 0)
                                @foreach ($general_rfqs as $general_rfq)
                                    <tr>
                                        <td>{{ ($loop->index + 1) }}</td>
                                        <td>{{ $general_rfq->user?$general_rfq->user->name:'Anonymous'}}</td>
                                        <td>{{ $general_rfq->user?$general_rfq->user->phone:'Anonymous'}}</td>
                                        <td>{{ $general_rfq->user?$general_rfq->user->email:'Anonymous' }}</td>
                                        <td>{{ $general_rfq->created_at->format('d-M-Y') }}</td>
                                        <td>{{ date('h:i A', strtotime($general_rfq->created_at)) }}</td>
                                        <td>
                                            <a href="{{route('company.products.rfq.details', $general_rfq->id)}}"
