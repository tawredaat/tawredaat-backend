...
@section('reports-moreInfo-active', 'm-menu__item--active m-menu__item--open')
@section('page-title', 'Reports|More info btn clicks')
@section('content')
    <style type="text/css">
        .swal2-confirm {
                        ...
                        <div class="m-portlet__head-caption">
                            <div class="m-portlet__head-title">
                                <h3 class="m-portlet__head-text">
                                    View all more info btn clicks from users
                                </h3>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped- table-bordered table-hover table-checkable"
                            ...
                            @if(count($MoreInfo) > 0)
                                @foreach ($MoreInfo as $info)
                                    <tr>
                                        <td>{{ ($loop->index + 1) }}</td>
                                        <td>{{ $info->user?$info->user->name:'Anonymous'}}</td>
                                        <td>{{ $info->user?$info->user->phone:'Anonymous'}}</td>
                                        <td>{{ $info->user?$info->user->email:'Anonymous'}}</td>
                                        <td>{{ $info->created_at->format('d-M-Y') }}</td>
                                        <td>{{ date('h:i A', strtotime($info->created_at)) }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <td colspan="7" class="text-center">There are no clicks in more info btn.</td>
                            @endif
