...
@section('page-title', $company->products_title)
@section('page-keywords', $company->products_keywords)
@section('page-description',$company->products_description)
...
<?php
$ar_route = route('user.company-products',['name'=>str_replace([' ','/'], '-',$company->translate('ar')->name),'id'=>$company->id]);
$seg = request()->segment(1);
$ar_route = str_replace($seg, 'ar/'.$seg,$ar_route);
?>
@section('alternate-ar-link',$ar_route)
@else
    @section('alternate-ar-link', url()->current())
<?php
$en_link = str_replace("/ar", "",route('user.company-products',['name'=>str_replace([' ','/'], '-',$company->translate('en')->name),'id'=>$company->id]));
?>
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">
          <a href="{{route('user.company',['name'=>str_replace([' ','/'], '-',$company->translate('en')->name),'id'=>$company->id])}}">
            <h1>
              {{$company->name}}
            </h1>
          </a>
