...
{{-- if this product in user cart  --}}
    @if($product->inCart)
        ...
        <a title="remove item" style="font-weight: bold;" data-route="{{route('user.delete.cart')}}" data-cartitemid="{{$product->cart_item_id}}"  data-shopproductid="{{$product->id}}"   class="yellow-rounded yellow-fill m-2 add-btn-holder delete-from-cart-btn">
...
{{-- if this product not in user cart  --}}
    @else
        ...
        @if(isLogged())
                <a style="font-weight: bold;" data-route="{{route('user.add.cart')}}" data-shopproductid="{{$product->id}}"   class="yellow-rounded yellow-fill m-2 add-to-cart-btn add-btn-holder">
                    <img width="44" class="mr-2 d-inline" src="{{ asset('frontend_plugins/web/images/shopping-cart.svg')}}" >@lang('home.AddToCart')</a>
        @else
            <a style="font-weight: bold;" data-toggle="modal" data-target="#LoginModal"  class="yellow-rounded yellow-fill m-2 add-btn-holder">
            <img width="44" class="mr-2 d-inline" src="{{ asset('frontend_plugins/web/images/shopping-cart.svg')}}" >@lang('home.AddToCart')</a>
        @endif
    @endif
