...
@section('dynamic-page-view-active', 'm-menu__item--active')
@section('page-title', 'Dynamic Page | View')
@section('content')
    <style type="text/css">
        .swal2-confirm {
                            ...
                            <div style="display: flex;justify-content: flex-end;align-items:flex-end;margin-bottom: 20px">
                                ...
                                <div class="form-group m-form__group col-6">
                                    <div class="input-group" style="width: 450px">
                                        @if ($shop_count >= 500)
                                            <label style="padding: 10px;"View 500 Page :</label>
                                        @endif
                                        ...
                                        <div class="input-group-append">
                                            <select id="searchColumn" class=" form-control" data-live-search="true"
                                                ...
                                                <option value="name">Dynamic Page Name</option>
                                            </select>
                                        </div>
                                        <div class="input-group-append">
                                    ...
                                    <form method="post" id="deleteForm"
                                        ...
                                        <div class="form-group m-form__group">
                                            <button class="btn btn-danger" type="submit">
                                                Delete Selected <span id="deleteCount">0</span> Page
                                            </button>
                                        </div>
                                    </form>
                                    ...
                                    <form method="post" id="deleteFilteredForm" {{-- action="{{ route('shop.products.delete.selected') }}" --}}
                                        ...
                                        <div class="form-group m-form__group">
                                            <button class="btn btn-danger" id="deleteFilteredBtn" type="button">
                                                Delete Filtered Brands
                                            </button>
                                        </div>
                                    </form>

                                    <form method="post" id="deleteAll" action="{{ route('bundels.delete.all') }}">
                                        @csrf
                                        <div class="form-group m-form__group">
    ...
    @push('script')
        <script type="text/javascript">
            ...
            $("#products-table").on('click', '.delete', function() {
                Swal.fire({
                    title: 'Are you sure to delete this bundel?',
                    ...
                    if (result.value) {
                        ...
                        var urls = "{{ route('bundels.destroy', 'id') }}";
            ...
            $("#products-table").on('click', '.featured', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data['featured'] == 1) {
                            $(aTag).css('color', 'red');
                            $(aTag).attr('title', 'Remove this bundel from featured in home page');
                        } else if (data['featured'] == 0) {
                            $(aTag).css('color', '#fff');
                            $(aTag).attr('title', 'Make this bundel featured in home page');
                        }
            ...
            $("#products-table").on('click', '.toggleShow', function() {
                var content = $(this).data("content");
                var urls = "{{ route('dymanic-page.show', 'id') }}";
                urls = urls.replace('id', content);
                $.ajax({
                    url: urls,
                    method: 'POST',
                    ...
                    success: function(data) {
                        ...
                        if (data['show'] == 1) {
                            ...
                            $(aTag).attr('title', 'Hide this bundel from website');
                        } else if (data['show'] == 0) {
                            $(iTag).attr('class', 'fa fa-eye-slash');
                            $(aTag).attr('title', 'Show this bundel in website');
                        }
                        var msg = data['success'];
                        toastr.success(msg);
                    },
            ...
            render("{!! route('dymanic-page.index', [
                'l3' => request()->input('l3'),
...
//show selected
            ...
            async function deleteAllSure(e) {
                e.preventDefault();
                return Swal.fire({
                    title: 'Are you sure to delete all bundels?',
                    text: "You won't be able to revert this!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
