...
class ShopProductResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if ($user != false && isset($user->items)
            ...
            $in_cart = 1;
            $product_cart_item_id = $user->items->where('shop_product_id', $this->id)->where('cart_id', $user->cart->id)->first()->id;
            $product_cart_item_qty = $user->items->where('shop_product_id', $this->id)->where('cart_id', $user->cart->id)->first()->qty;
        } else {
            //for guest cart item
            $cart = [];
        ...
        return [
            'id' => intval($this->id), 'inCart' => intval($in_cart),
            'name' => $this->name,
            'slug' => $this->slug(),
            ...
            'note' => $this->note,
            'seller' => $this->seller,
            'pdf' => $this->pdf ? str_replace('\\', '/', asset('storage/' . $this->pdf)) : null,
            'sku_code' => $this->sku_code,
            'alt' => $this->alt,
            'description' => $this->description,
            'old_price' => "$this->old_price",
            'new_price' => "$this->new_price",
            'quantity_type' => $this->QuantityType ? $this->QuantityType->name : '',
            'qty' => $this->qty,
			'bundel_qty' => ($this->quantity ?? 0) > $this->qty ? $this->qty : ($this->quantity ?? null),
            'from' => $this->from,
            'to' => $this->to,
            'out_of_stock' => (bool) ($this->qty == 0),
            'brand' => new BrandResource($this->brand),
            'specifications' => new ShopProductSpecificationsCollection($this->specifications),
            'show' => $this->show ,
          	'offer' => $this->offer,
